...
RegisterCommand('+radiotalk', function()
	radio2 = QBCore.Functions.HasItem('radio')
	radio1 = QBCore.Functions.HasItem('walkie_talkie')
	phone = QBCore.Functions.HasItem('phone')
	dongle = QBCore.Functions.HasItem('radio_dongle')
	print(radio1,radio2,phone,dongle)
	if GetConvarInt('voice_enableRadios', 1) ~= 1 then return end
	if isDead() or LocalPlayer.state.disableRadio then return end

	if not radioPressed and radioEnabled and ((radio1 or radio2) or (dongle and phone)) then
		if radioChannel > 0 then
			...
			if GetConvarInt('voice_enableRadioAnim', 0) == 1 and not (GetConvarInt('voice_disableVehicleRadioAnim', 0) == 1 and IsPedInAnyVehicle(PlayerPedId(), false)) and not disableRadioAnim then
				...
				if phone and dongle then 
					RequestAnimDict('eagle@boypose04')
					while not HasAnimDictLoaded('eagle@boypose04') do
						Wait(10)
					end
					TaskPlayAnim(PlayerPedId(), "eagle@boypose04", "boy", 8.0, 2.0, -1, 50, 2.0, false, false, false)
					PhoneProp = CreateObject(`prop_phone_ing`, 1.0, 1.0, 1.0, 1, 1, 0)
					AttachEntityToEntity(PhoneProp, PlayerPedId(), GetPedBoneIndex(PlayerPedId(), 18905), 0.12, 0.03, 0.02, 170.0, 90.0, 50.0, 1, 0, 0, 0, 2, 1)
				end
			end
